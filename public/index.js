...
function change_subnet_number() {
  ...
  c = c.replace(/^\s+|\s+$/g, "");
  if (isNaN(c)) {
    alert("Por favor ingresa un valor entre 2 y 999");
  } else {
    if (c % 1 !== 0) {
      alert("Ingrese un número entero, sin decimales ni comas");
    } else {
      if (c > 999 || c < 2) {
        alert("Ingrese un número entero positivo entre 2 y 999");
      } else {
        paragraph =
          "<span class='column'>Subredes:</span><span class='column' style='padding-left: 120px;'>Cantidad:</span><br>";
        for (var b = 1; b <= c; b++) {
...
function vlsm() {
  ...
  if (!validate(h)) {
    document.getElementById("not_valid_ip").innerHTML =
      "<b>Esto no parece una red válida</b>";
    return;
  }
  document.getElementById("not_valid_ip").innerHTML = "";
  ...
  var s =
    ...
    " tiene " +
    g +
    " hosts.<br>Las subredes necesitan " +
    a +
    " hosts.</p>";
  var t = "<table border='1'><tr><td>Subred</td><td>IP de red</td><td>Mascara</td><td>Rango usable</td><td>Broadcast</td><td>Wildcard</td></tr>";
  var b = c;
  var k = 0;
  ...
  if (k > g + 2) {
    s +=
      "<span style='background-color:yellow;'>Parece que esas subredes no encajarán en esa red, pero aquí hay algo más que puede funcionar para usted:</span><br>";
  }
  t = t;
